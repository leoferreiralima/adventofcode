...
pub fn main() !void {
    ...
    while (updateLines.next()) |line| {
        ...
        std.mem.sort(usize, seen.items, rulesMap, lessThan);
...
fn lessThan(rules: std.AutoHashMap(usize, []usize), a: usize, b: usize) bool {
    const @"a|b" = rule: {
        const rule = rules.get(a) orelse break :rule true;
        break :rule if (std.mem.indexOfScalar(usize, rule, b)) |_| true else false;
    };
    const @"b|a" = rule: {
        const rule = rules.get(b) orelse break :rule false;
        break :rule if (std.mem.indexOfScalar(usize, rule, a)) |_| true else false;
    };
    return @"a|b" and !@"b|a";
}
